...
`order-tracking-api` (scatter + список), `order-tracking-worker`, затем кадр из **Playwright** в репозитории.

![Jaeger: поиск order-tracking-api — scatter и список](screenshots/traces-jaeger-search-api-scatter.png)

![Jaeger: поиск order-tracking-api — фрагмент списка](screenshots/traces-jaeger-search-api-list.png)

![Jaeger: поиск order-tracking-worker](screenshots/traces-jaeger-search-worker.png)

![Jaeger: поиск (автосъёмка Playwright)](screenshots/jaeger-search.png)
